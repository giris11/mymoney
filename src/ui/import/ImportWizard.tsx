...
export default function ImportWizard({ onDone, onCancel }: ImportWizardProps) {
  ...
  async function handleFile(f: File) {
    ...
    try {
      ...
      if (isMoneyWizCsv(headers)) {
        ...
        const saved = settings.savedMappings[fileSignature(headers)];
        setFile({
          name: f.name,
          sizeBytes: f.size,
          data,
        ...
        setMapping(saved ? { ...saved } : guessMapping(headers, data.slice(1, 11)));
        setStep('map');
      }
    } catch (e) {
    ...
    try {
      ...
      await updateSettings({
        savedMappings: {
          ...
          [fileSignature(file.headers)]: mapping,
        },
      });
